...
## Слайд 10. Что такое ФФ и какую задачу они решают
...
## Слайд 11. High Level Architecture
...
## Слайд 25. Фиче-флаг для 2 команд разработки (Задача 2: распределенная разработка)
...
## Слайд 35. Переход из Self-hosted к Cloud Native (Задача 1: деплоймент в разные среды)
...
## Слайд 40. Задача 4: миграция кодовой базы
...
- [x] перерисовать картинку хореография vs оркестрация
...
- [x] добавить qr код в последнем файле
- [x] 22 - кейс? до этого убрать user и тд 
- [x] 29 - можно показать конкретный кейс, можно рассказать, почему флаги сразу в микрофронт не прокидывается
- [x] 32 - уход в сторону, можно убрать
- [x] 39 - надо визуализировать
- [x] 43 - объединяем - показать пресеты
- [x] 45 - задача 1: Решение - что можешь посоветовать из опенсурса
    - задача 3: не понял, причем тут фича флаги
- [x] 49 - после обл провайдеров, может не подойти - почему может не подойти?
    - разработали либу - нет.
- [x] 52 - можно показать пример теста
- [x] 53 - баги в удаленном сервисе. Удаление - не про тесты. 
    - код тестов
